...
describe('PostsChooseNewPostCategory Component', () => {
  ...
  it('PostsChooseNewPostCategory Component - onToCreateIdea: should navigate to idea-createIdeaQuestion page', fakeAsync(() => {
    spy = spyOn(component, 'onToCreateIdea').and.callThrough();
    component.onToCreateIdea();
    flushMicrotasks();
    expect(spy).toHaveBeenCalled();
    spy = spyOn(component._nav, 'push').and.callThrough();
    component.onToCreateIdea();
    flushMicrotasks();
    expect(spy).toHaveBeenCalledWith('idea-createIdeaQuestion');
    spy.calls.mostRecent().returnValue.then((result) => {
      expect(result).toEqual('PUSH');
    });
  }));
